...
The function to compute the 3d point, given a distance measurement $d$,
the rotational position $\theta$ and the devica laser id $l$ is close to
the formula ($\Delta d$ is explained below):
\begin{eqnarray}
...
\end{eqnarray}
with $p'(\cdot) = \left(p_x'(\cdot) , p_y'(\cdot) , p_z'(\cdot) \right)^T$.
