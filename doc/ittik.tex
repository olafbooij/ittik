...
We first apply the algorithm on a KITTI scan (see Figure~\ref{fig:example} to get the raw measurments and then
...
the example scan is 3.29 mm. Perhaps more interesting is the mean error of the
distance of the point to the sensor: .85 mm. The angle of the point in the
horizontal plane of the sensor is the same for the original and the
reconstructed point. 
