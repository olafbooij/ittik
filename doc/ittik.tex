...
each laser fire from the lased Id, the measured distance and measured
...
device. There are, however, also device specific parameters, which are
determined in the factory using a specific calibration procedure and
stored on the device. These values are also transmitted by the device to
be stored and used by the preprocessing procedure.
...
% about XX times larger than the format used by the sensor.
