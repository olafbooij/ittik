...
the device is rotating at a rate of 10 Hz, resulting in approx. 1.3
million measurements per second. The lasers are distributed over 4
locations with a slightly different offset. In addition, the rays of
each laser have a different vertical and horizontal direction. The exact
value of these offsets and directions are given in the calibration
stored on the device (more on this in the next section). In
Figure~\ref{fig:directions} the different directions of the laser rays
are shown. As can be seen the vertical view-angle is approximately 28
degrees and the horizontal view-angle 19 degrees.
